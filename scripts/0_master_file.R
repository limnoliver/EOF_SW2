...
site <- 'SW2' # site abbreviation that may be used for file naming conventions
...
######## BMP timeline ############
start_date <- as.POSIXct('2013-01-10 00:00:01') # YYYY-MM-DD HH:MM:SS date/time of study start
                             # This can be the first second of the day on the date of the first sampling event
bmp_date <- as.POSIXct('2015-11-17 00:00:01') # YYYY-MM-DD HH:MM:SS date/time of BMP implementation. All events before
                           # this date will be considered 'before', all events after this 
                           # date will be considered 'after'
end_date <- as.POSIXct('2018-09-30 00:00:01') # YYYY-MM-DD HH:MM:SS date/time of study end
...
loads <- 'load_pounds' # this can either be a vector of verbatim constituent load
# columns (e.g, c('SS_load_pounds', 'TP_load_pounds')) or a character string that all load
# columns share in common (e.g., "pounds"). 

other_responses <- 'peak_discharge' # this is a space for a response variable that may not be a constituent concentration or load. 
# e.g., if you expect the BMP to affect runoff itself, you may want to assess total event runoff or peak discharge 
# as a response variable.

flags <- NA # this can either be a vector of verbatim flag
...
event_over_thaw <- 'non-frozen' # For subevents that span a thaw, you may have to decide whether to split the events or combine them.
...
######## rain data ###########
rain_site <-'441546088082001'  # site number for rain gage if pulling from NWIS, 'NA' if providing data file
