...
wq$sample_start <- as.POSIXct(wq$sample_start, origin = .origin, tz = site_tz, format = datetime_format)
wq$sample_end <- as.POSIXct(wq$sample_start, origin = .origin, tz = site_tz, format = datetime_format)
...
write.csv(wq, 'data_cached/wq_dates_fixed.csv', row.names = FALSE)
