...
for (i in 1:length(responses)) {
  ...
  pdf(paste0('figures/withpd/', site, '_rf_pp_', responses[i], '.pdf'), heigh = 6, width = 6)
  par(mfcol = c(2,2), mar = c(4,2,2,2), oma = c(2,2,3,0))
  for (k in top.vars){
    partialPlot(mod, pred.data = dat.mod, x.var = paste(k),
                xlab = k, main = "")
  }
  ...
  fig.name = paste0('figures/withpd/', site, '_rf_modsum_', responses[i], '.pdf')
...
temp_filename <- file.path('data_cached/withpd/', paste0(site, '_before_after_residual_analysis.csv'))
...
temp_filename <- file.path('data_cached/withpd/', paste0(site, '_percent_reduction_before_after.csv'))
